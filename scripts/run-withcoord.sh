...
do
	host=`echo $machines | cut -d ',' -f$x`
    nodeId=$((x-1))    
    (ssh $host "/tmp/cli-timeout $nodeId $ip_list $port_list $role_list $size" &)
done
...
do
	host=`echo $machines | cut -d ',' -f$x`
    nodeId=$((x-1))    
    (ssh $host "/tmp/cli-timeout $nodeId $ip_list $port_list $role_list $size $ASYNC_MSGS 20 1" &)
done

host=`echo $machines | cut -d ',' -f$coordloc`
ssh $host "/tmp/cli-timeout $clientend $ip_list $port_list $role_list $size"
