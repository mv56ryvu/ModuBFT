...
// Completetd send callback
void send_cb(void *request, ucs_status_t status, void* user_data) {
	printf("Finished sending message to DPU %d\n", status);
	free(user_data);
}
...
void ucx_client::send(std::string data, int messageId) {
	...
	printf("Got send for message with %ld bytes with id %d\n", data.size(), messageId);

	char* msg = (char*) malloc(data.size());
	memcpy(msg, data.c_str(), data.size());
	...
	if(messageId < 2) {
		request = ucp_am_send_nbx(myEndpoint, messageId, msg, data.size(), NULL, 0, &params);
	} else {
		request = ucp_am_send_nbx(myEndpoint, messageId, NULL, 0, msg, data.size(), &params);
	}

	if(request == NULL) {
		// Request was completed immediately, so we free msg now as send_cb won't be called
		free(msg);
	}

	printf("Finished send for message with %ld bytes with id %d\n", data.size(), messageId);
